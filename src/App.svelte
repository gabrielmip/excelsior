<script>
	import {parse, parser, AssignmentNode} from 'mathjs';
	...
	afterUpdate(() => {
		if (chart) {
			const newData = expressions.map(({symbol}) => symbol in scope ? scope[symbol] : null);
		...
		chart = new Chart(canvas, {
			...
			data: {
				...
				datasets: [{
					...
					data: expressions.map(({symbol}) => symbol in scope ? scope[symbol] : null),
				}]
			},
			options: {
				scales: {
	...
	function persistParsedInput (parsed) {
		...
		if (parsed.isAssignmentNode) {
			...
			try {
				...
				const expression = expressions.find(({symbol}) => symbol === parsed.name);
				if (expression) {
	...
	function getSymbolNames (parsed) {
		return parsed.filter(({isSymbolNode}) => isSymbolNode).map(({name}) => name);
	}

	function updateScopeStartingFromExpression (expression, currentScope, expressions) {
		...
		for (const dep of expression.dependencies) {
			...
			if (depExpression) {
				currentScope = updateScopeStartingFromExpression(
					depExpression, currentScope, expressions);
			}
		}
	...
	function updateScope () {
		scope = expressions.reduce(
			(currentScope, expression) =>
				updateScopeStartingFromExpression(expression, currentScope, expressions),
			{}
		);
	}
...
<button on:click={() => print({code: 'Enter'})}>
	Adicionar
</button>
...
<div class="formulas">
	{#each expressions as exp}
		<div class="formula-container" on:doubleclick={() => console.log("hi")}>
			{exp.symbol} =
			...
			{#if symbolsBeingEdited[exp.symbol] === undefined}
				...
				{:else}
					...
					<br>
					Calculado:
					{#if exp.symbol in scope}
						{scope[exp.symbol].toFixed(2)}
					{:else}
						-
					{/if}
				{/if}
				...
				{#each missingDependenciesBySymbol[exp.symbol] as dep}
					{dep},
				{/each}
				<button
					on:click={() =>
						instantiateDependencies(missingDependenciesBySymbol[exp.symbol])}>
				Definir
				</button>
				{/if}
			{:else}
